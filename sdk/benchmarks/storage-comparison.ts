...
import type { StorageAdapter, StoredDocument } from '../dist/index.mjs'
...
function generateDocument(id: string, size: 'small' | 'medium' | 'large'): StoredDocument {
  const sizeMap = {
    small: 10,    // 10 todos
...
async function runBenchmarks() {
  ...
  for (const { name, adapter } of adapters) {
    ...
    try {
      ...
      const readDocs = Array.from({ length: iterations }, (_, i) =>
        generateDocument(`read-doc-${i}`, 'medium')
      )
      ...
      const deleteDocs = Array.from({ length: iterations }, (_, i) =>
        generateDocument(`delete-doc-${i}`, 'small')
      )
...
function exportResults(results: BenchmarkResult[]): string {
...
if (typeof window !== 'undefined') {
  ...
  ;(window as any).runStorageBenchmarks = async () => {
